...
if "DIR" not in st.session_state:
    st.session_state[
        "DIR"
    ] = "D:/001_zerlingx/notes/literature/HC/007_experiments/2023-07 一号阴极测试/2023-08-30 点火与单探针测试/data/RAW/"
...
@st.cache_data()
def plot_data_points(plot_data, plot_range, fre_range):
    fig, ax = basic_plot.plot_curve_and_FFT(
        data_points=plot_data, plot_range=plot_range, title=path, fre_range=fre_range
    )
    return fig
...
try:
    ...
    fig = plot_data_points(data_points, plot_range, fre_range)
    st.pyplot(fig)
